...
import SubscribersRecord from "./SubscribersRecord";
import { useGetAllSubscribers } from "@/lib/api/subscription";

interface UserTableProps {
  searchTerm: string;
}

const SubscribersTable = ({ searchTerm }: UserTableProps) => {
  const [currentPage, setCurrentPage] = useState(1);

  const deboucedSearchValue = useDebounce(searchTerm, 2000);

  const { data, isLoading, error, isError } = useGetAllSubscribers({
    per_page: "20",
  ...
  return (
    <div className="bg-white">
      ...
      >
        <AppTable
          columns={SubscribersRecord}
          data={data?.data as ISubscribers[]}
        />
      </DataLoader>
